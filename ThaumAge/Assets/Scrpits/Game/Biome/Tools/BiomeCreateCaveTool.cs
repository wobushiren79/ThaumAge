...
public class BiomeCreateCaveTool : ScriptableObject
{
    ...
    public static void AddCave(Chunk chunk, BiomeForCaveData caveData)
    {
        float frequency = 2;
        float amplitude = 1;
        ...
        for (int i = 8; i < caveNumber; i++)
        {
            ...
            int positionY = WorldRandTools.Range(1, chunk.chunkData.chunkHeight);
            ...
            for (int f = 0; f < caveDepth; f++)
            {
                ...
                if (startPosition.y <= 3 || startPosition.y > chunk.chunkData.chunkHeight)
                    break;
