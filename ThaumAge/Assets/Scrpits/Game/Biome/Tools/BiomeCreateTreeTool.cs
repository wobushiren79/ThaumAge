...
public class BiomeCreateTreeTool
{
    ...
    /// 增加高树树 高度最低不能小于6
    /// </summary>
    public static void AddTreeForTall(int blockId, Vector3Int startPosition,int minHeight,int maxHeight,int leavesId,int leavesRange)
    {
        //高度
        int treeHeight = WorldRandTools.Range(minHeight, maxHeight);
        for (int i = 0; i < treeHeight + 2; i++)
        {
            Vector3Int treeTrunkPosition = new(startPosition.x, startPosition.y + (i + 1), startPosition.z);
            //生成树干
            if (i < treeHeight)
            {
                WorldCreateHandler.Instance.manager.AddUpdateBlock(treeTrunkPosition, blockId);
            }
            //再靠近树叶的地方随机生成一个树枝
            if (i == treeHeight - 6)
            {
                GetRandomBlockDirection(out BlockDirectionEnum randomBlockDirection, out Vector3Int randomAddPosition);
                WorldCreateHandler.Instance.manager.AddUpdateBlock(treeTrunkPosition + randomAddPosition, blockId, randomBlockDirection);
            }
            if (i > treeHeight - 4)
            {
                //最大范围
                ...
                for (int x = -range; x <= range; x++)
                {
                    for (int z = -range; z <= range; z++)
                    {
                        ...
                        WorldCreateHandler.Instance.manager.AddUpdateBlock(treeTrunkPosition.x + x, treeTrunkPosition.y, treeTrunkPosition.z + z, leavesId);
                    }
                }
            }
        }
    ...
    /// <param name="startPosition"></param>
    /// <param name="treeData"></param>
    public static void AddTreeForBig(int blockId, Vector3Int startPosition, int minHeight, int maxHeight, int leavesId, int leavesRange)
    {
        //高度
        int treeHeight = WorldRandTools.Range(minHeight, maxHeight);
        ...
        {
            //生成树叶
            Vector3Int treeTrunkPosition = startPosition + Vector3Int.up * i;
            int range = leavesRange;
            if (i == treeHeight + 2)
            {
                range -= 1;
        ...
        for (int i = 0; i < treeHeight; i++)
        {
            ...
            if (i == 0 || i == 1)
            {
                if (i == 0)
                {
                    WorldCreateHandler.Instance.manager.AddUpdateBlock(treeTrunkPosition.x - 2, treeTrunkPosition.y, treeTrunkPosition.z, blockId, BlockDirectionEnum.LeftForward);
                    WorldCreateHandler.Instance.manager.AddUpdateBlock(treeTrunkPosition.x + 2, treeTrunkPosition.y, treeTrunkPosition.z, blockId, BlockDirectionEnum.RightForward);
                    WorldCreateHandler.Instance.manager.AddUpdateBlock(treeTrunkPosition.x - 2, treeTrunkPosition.y, treeTrunkPosition.z + 2, blockId, BlockDirectionEnum.ForwardForward);
                    WorldCreateHandler.Instance.manager.AddUpdateBlock(treeTrunkPosition.x - 2, treeTrunkPosition.y, treeTrunkPosition.z - 2, blockId, BlockDirectionEnum.BackForward);
                }
                WorldCreateHandler.Instance.manager.AddUpdateBlock(treeTrunkPosition.x + 1, treeTrunkPosition.y, treeTrunkPosition.z + 1, blockId);
                WorldCreateHandler.Instance.manager.AddUpdateBlock(treeTrunkPosition.x - 1, treeTrunkPosition.y, treeTrunkPosition.z - 1, blockId);
                WorldCreateHandler.Instance.manager.AddUpdateBlock(treeTrunkPosition.x + 1, treeTrunkPosition.y, treeTrunkPosition.z - 1, blockId);
                WorldCreateHandler.Instance.manager.AddUpdateBlock(treeTrunkPosition.x - 1, treeTrunkPosition.y, treeTrunkPosition.z + 1, blockId);
            }
            ...
            if (i > treeHeight - 3)
            {
                ...
                if (isCreate == 1)
                {
                    WorldCreateHandler.Instance.manager.AddUpdateBlock(treeTrunkPosition.x, treeTrunkPosition.y, treeTrunkPosition.z + 2, blockId, BlockDirectionEnum.ForwardForward);
                }
                isCreate = WorldRandTools.Range(0, 4);
                if (isCreate == 1)
                {
                    WorldCreateHandler.Instance.manager.AddUpdateBlock(treeTrunkPosition.x, treeTrunkPosition.y, treeTrunkPosition.z - 2, blockId, BlockDirectionEnum.BackForward);
                }
            }
            WorldCreateHandler.Instance.manager.AddUpdateBlock(treeTrunkPosition.x, treeTrunkPosition.y, treeTrunkPosition.z, blockId);
            WorldCreateHandler.Instance.manager.AddUpdateBlock(treeTrunkPosition.x - 1, treeTrunkPosition.y, treeTrunkPosition.z, blockId);
            WorldCreateHandler.Instance.manager.AddUpdateBlock(treeTrunkPosition.x + 1, treeTrunkPosition.y, treeTrunkPosition.z + 1, blockId);
            WorldCreateHandler.Instance.manager.AddUpdateBlock(treeTrunkPosition.x, treeTrunkPosition.y, treeTrunkPosition.z + 1, blockId);
            WorldCreateHandler.Instance.manager.AddUpdateBlock(treeTrunkPosition.x, treeTrunkPosition.y, treeTrunkPosition.z - 1, blockId);
        }
    }
    ...
    /// 增加世界之树
    /// </summary>
    public static bool AddTreeForWorld(int blockId, Vector3Int startPosition, int minHeight, int maxHeight, int leavesId)
    {
        //高度
        int treeHeight = WorldRandTools.Range(minHeight, maxHeight);
            ...
            for (int x = -trunkRange; x <= trunkRange; x++)
            {
                for (int z = -trunkRange; z <= trunkRange; z++)
                {
                    ...
                    else
                    {
                        ...
                        //生成树干
                        WorldCreateHandler.Instance.manager.AddUpdateBlock(tempTrunkPosition, blockId);
                    }
                        ...
                        || ((z == -trunkRange + 2 || z == trunkRange - 2) && (x == -trunkRange || x == trunkRange)))
                    {
                        ...
                        if (y == 0 || y == 1)
                        {
                            ...
                            for (int b = 0; b < baseWith; b++)
                            {
                                ...
                                //干
                                WorldCreateHandler.Instance.manager.AddUpdateBlock(baseStartPosition, blockId, baseDirection);
                            }
                        }
                        ...
                        if (y > treeHeight / 2)
                        {
                            ...
                            if (addBranchRate == 0)
                            {
                                ...
                                for (int b = 0; b < branchWith; b++)
                                {
                                    ...
                                    //干
                                    WorldCreateHandler.Instance.manager.AddUpdateBlock(branchStartPosition, blockId, branchDirection);
                                    //叶
                                    if (b % 4 == 0)
                                    {
                                        int leavesRange = 2;
                                        for (int leavesX = -leavesRange; leavesX <= leavesRange; leavesX++)
                                        {
                                            for (int leavesY = -leavesRange; leavesY <= leavesRange; leavesY++)
                                            {
                                                for (int leavesZ = -leavesRange; leavesZ <= leavesRange; leavesZ++)
                                                {
                                                    ...
                                                    WorldCreateHandler.Instance.manager.AddUpdateBlock(leavesPosition, leavesId);
                                                }
                                            }
                                        }
                                    }
    ...
    public static void AddTreeForWorldEditor(Vector3Int startPosition, BiomeForTreeData treeData)
    {
        ...
        for (int y = 0; y < treeHeight; y++)
        {
            Vector3Int treeTrunkPosition = startPosition + Vector3Int.up * y;
    ...
    /// 增加柳树
    /// </summary>
    public static bool AddTreeForSalix(int blockId, Vector3Int startPosition,
        int minHeight, int maxHeight, 
        int leavesId, int leavesRange,
        int leavesMinH, int leavesMaxH)
    {
        //高度
        int treeHeight = WorldRandTools.Range(minHeight, maxHeight, startPosition);
            ...
            for (int x = -range; x <= range; x++)
            {
                for (int z = -range; z <= range; z++)
                {
                    ...
                    if (i == treeHeight - 2)
                    {
                        ...
                        if (randomCreate == 0)
                        {
                            int wickerLenth = WorldRandTools.Range(leavesMinH, leavesMaxH);
        ...
        for (int i = 0; i < treeHeight; i++)
        {
            ...
            if (randomCreate == 1)
            {
                ...
                {
                    case 0:
                        WorldCreateHandler.Instance.manager.AddUpdateBlock(treeTrunkPosition.x - 1, treeTrunkPosition.y, treeTrunkPosition.z + createDirectionOffset, blockId, BlockDirectionEnum.LeftForward);
                        break;
                    case 1:
                        WorldCreateHandler.Instance.manager.AddUpdateBlock(treeTrunkPosition.x + 2, treeTrunkPosition.y, treeTrunkPosition.z + createDirectionOffset, blockId, BlockDirectionEnum.RightForward);
                        break;
                    case 2:
                        WorldCreateHandler.Instance.manager.AddUpdateBlock(treeTrunkPosition.x + createDirectionOffset, treeTrunkPosition.y, treeTrunkPosition.z - 1, blockId, BlockDirectionEnum.ForwardForward);
                        break;
                    case 3:
                        WorldCreateHandler.Instance.manager.AddUpdateBlock(treeTrunkPosition.x + createDirectionOffset, treeTrunkPosition.y, treeTrunkPosition.z + 2, blockId, BlockDirectionEnum.BackForward);
                        break;
                }
            }
            WorldCreateHandler.Instance.manager.AddUpdateBlock(treeTrunkPosition.x, treeTrunkPosition.y, treeTrunkPosition.z, blockId);
