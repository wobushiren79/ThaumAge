		...
		public void Execute () {
			...
			for (int y = 1; y < size.y; y++) {
				for (int z = 0; z < size.z; z++) {
					...
					var offset2 = ((y + dataBounds.min.y) * nodeArrayBounds.z + (z + dataBounds.min.z)) * nodeArrayBounds.x;
					for (int x = 0; x < size.x; x++) {
						var shouldHaveNode = math.any(nodeNormals[offset1 + x]);
						var nodeIndex = offset2 + x + dataBounds.min.x;
				...
				for (int z = relativeMask.min.z; z < relativeMask.max.z; z++) {
					...
					for (int x = relativeMask.min.x; x < relativeMask.max.x; x++) {
						...
						if (node != null) {
							...
							node.NodeInGridIndex = ((y + dataBounds.min.y) << LevelGridNode.NodeInGridIndexLayerOffset) + rowOffset2 + x;
